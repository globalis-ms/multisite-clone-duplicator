...
if ( ! class_exists( 'Site_Duplicate_Subcommand' ) && class_exists( 'MUCD_Functions' ) && class_exists( 'MUCD_Clone_Site' ) ) :
	...
	class Site_Duplicate_Subcommand extends WP_CLI_Command {
		...
		public function __invoke( $_, $assoc_args ) {
			...
			// Copy files ?
			$copy_files = isset( $assoc_args['do_not_copy_files'] ) ? false : true;

			// Keep users ?
			$keep_users = isset( $assoc_args['keep_users'] ) ? true : false;
			...
			$wpdb->hide_errors();
			$form_message = MUCD_Clone_Site::clone_site( $data );
			$wpdb->show_errors();
