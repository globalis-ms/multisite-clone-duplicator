...
if ( ! class_exists( 'MUCD_Admin' ) ) {
	...
	class MUCD_Admin {
		...
		public static function network_menu_add_duplicate() {
			add_submenu_page( 'sites.php', MUCD_NETWORK_PAGE_DUPLICATE_TITLE, MUCD_NETWORK_MENU_DUPLICATE,
				'manage_sites', MUCD_SLUG_NETWORK_ACTION, array( __CLASS__, 'network_page_admin_duplicate_site' ) );
		}
		...
		public static function select_site_list( $site_list, $current_blog_id = null ) {
			...
			} else if ( isset( $current_blog_id ) && MUCD_Functions::value_in_object( $current_blog_id, $site_list,
					'blog_id' ) && MUCD_Functions::is_duplicable( $current_blog_id )
			) {
			...
			foreach ( $site_list as $site ) {
				if ( isset( $blog_id ) && $site['blog_id'] == $blog_id ) {
					$output .= '    <option selected value="' . $site->blog_id . '">' . substr( $site->domain . $site->path,
							0, - 1 ) . '</option>';
				} else {
					$output .= '    <option value="' . $site->blog_id . '">' . substr( $site->domain . $site->path, 0,
							- 1 ) . '</option>';
				}
			}
			$output .= '</select>';
			...
			if ( check_admin_referer( MUCD_DOMAIN ) ) {
				...
				if ( ! is_subdomain_install() ) {
					...
					if ( in_array( $domain, $subdirectory_reserved_names ) ) {
						$error[] = new WP_Error( 'mucd_error',
							sprintf( MUCD_NETWORK_PAGE_DUPLICATE_DOMAIN_ERROR_RESERVED_WORDS,
